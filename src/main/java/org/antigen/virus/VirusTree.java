...
public class VirusTree {
  ...
  static final Comparator<Virus> descendantOrder =
      (v1, v2) -> {
    ...
    try {
      ...
      if (Parameters.phenotypeSpace.equals("geometricSeq")) {
        tipStream.printf(
            ...
            "ag1",
            "ag2",
            "epitopeMutationCount",
            "nonepitopeMutationCount",
            "lowEpitopeMutationCount",
            "highEpitopeMutationCount",
            "fitness",
            "averageInfectionRisk",
            "probSusceptible",
            "demeSeasonality");
      } else {
        tipStream.printf(
      ...
      for (Virus v : tips) {
        if (Parameters.phenotypeSpace.equals("geometricSeq")) {
          ...
          tipStream.printf(
              "\"%s\",%.4f,%d,%d,%d,%d,%.4f,\"%s\",%s,%s,%s,%s,%s,%s,%.4f,%.4f,%.4f,%.4f\n",
              v,
              v.getBirth(),
              v.isTrunk() ? 1 : 0,
              v.isTip() ? 1 : 0,
              ...
              highEpitopeMutationCount,
              v.getFitness(),
              v.getAverageInfectionRisk(),
              v.getProbSusceptible(),
              v.getDemeSeasonality());
        } else {
          tipStream.printf(
              "\"%s\",%.4f,%d,%d,%d,%d,%.4f,%s,%.4f,%.4f,%.4f,%.4f\n",
              v,
              v.getBirth(),
              v.isTrunk() ? 1 : 0,
              v.isTip() ? 1 : 0,
  ...
  public static void printMKSummary() {
    ...
    try {
      PrintStream summaryStream =
          new PrintStream(new FileOutputStream(filenameStem + ".summary", true)); // append
